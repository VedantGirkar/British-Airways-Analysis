...
def data_cleaning(df):
    ...
    df["Via"] = df["To"].str.split(" via ", n=1).str[1].fillna("Direct")
    df["To"] = df["To"].str.split(" via ", n=1).str[0].fillna("Unknown")
    
    df["Stops"] = df["Via"].notna().map({True: "1 stop", False: "Direct"})
    
    df["Seat Comfort"] = df["Seat Comfort"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64")
    
    df["Cabin Staff Service"] = df["Cabin Staff Service"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64") 
    
    df["Food & Beverages"] = df["Food & Beverages"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64") 
    
    df["Inflight Entertainment"] = df["Inflight Entertainment"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64") 
    
    df["Ground Service"] = df["Ground Service"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64") 
    
    df["Wifi & Connectivity"] = df["Wifi & Connectivity"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64") 
    
    df["Value For Money"] = df["Value For Money"].str.split("stars", n=1, expand=True)[0].fillna(0).astype("int64") 
    
    df["Recommended"] = df["Recommended"].map({"yes": True}).fillna(False)
...
def main():
    ...
    # data_exploration(df, "Food & Beverages")
    
    df = df.filter(items=["Date", 
                          "Rating", 
                          "Headline", 
                          "Verified", 
                          "Aircraft", 
